...
function confirmDelete() {
    const accountName = accountData.name || 'this account';
    return confirm(`Are you sure you want to delete ${accountName}? This action cannot be undone.`);
}
...
statNumbers.forEach(stat => {
    ...
    if (!isNaN(finalValue)) {
        let start = 0;
        const duration = 1000;
        const step = Math.ceil(finalValue / (duration / 30));
